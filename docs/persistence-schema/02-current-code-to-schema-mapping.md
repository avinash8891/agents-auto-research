...
## Exported experiment entry -> `experiments`

| Current field | Canonical target | Status | Notes |
|---|---|---|---|
| `job` | `experiments.job_id` | direct | |
| `run_id` | `experiments.run_id` | direct | |
| `commit` | `experiments.code_commit` | direct | |
| `metric` | `experiments.primary_metric_value` | direct | |
| config header `metricName` | `experiments.primary_metric_name` | direct | source is config header |
| `metrics` | `experiments.metrics_json` | direct | |
| `status` | `experiments.decision_status` | direct | currently only `keep` / `discard` |
| `timestamp` | `experiments.created_at_utc` | direct | |
| `asi.config` | `experiments.config_path` | direct | |
| `asi.trade_analysis` | `experiments.trade_analysis_json` | direct | |
| `asi.thesis_id` | `experiments.thesis_id` | direct | |

---

## Current `ExperimentResult` -> `experiments`

| Current field | Canonical target | Status | Notes |
|---|---|---|---|
| `experiment_id` | `experiments.experiment_id` | direct | |
| `thesis_id` | `experiments.thesis_id` | direct | |
| `config_path` | `experiments.config_path` | direct | |
| `runtime_config` | `experiments.runtime_config_json` | direct | |
| `code_commit` | `experiments.code_commit` | direct | |
| `data_hash` | `experiments.data_hash` | direct | |
| `train_metrics` + `validation_metrics` | `experiments.metrics_json` | direct_or_merge | current code splits them in JSON DB, canonical schema does not |
| `strategy_diagnostics` | `experiments.strategy_diagnostics_json` | direct | |
| `accepted` | `experiments.decision_status` | DERIVED | `True -> keep`, `False -> discard` |
| `verdict_status` | `experiments.verdict_status` | direct | |
| `verdict_summary` | `experiments.verdict_summary` | direct | |
| `timestamp` | `experiments.created_at_utc` | direct | |
| `family` | `experiments.strategy_family` | direct | |
| `job` | `experiments.job_id` | direct | |

### Important mapping limits
- `experiments.experiment_id` does not exist in exported entries and comes from `ExperimentResult` / DB path.
...
2. Never assume experiment rows already support `blocked`/`failed` statuses.
