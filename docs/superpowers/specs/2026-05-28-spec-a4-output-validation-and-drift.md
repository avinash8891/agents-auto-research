...
|---|---|---|---|---|---|---|
| Required process tools called. | process | `_validate_process` | `process_required_tools_not_called` | keep | Read the attempt trace. Require `list_round_results` before using prior-round claims, `web_search` before accepting `evidence_citations[source="web_search"]`, and `analyze_trades` before accepting analyst/trade-level claims required by A4a evidence guidance. Tool aliases must normalize to canonical tool names before comparison. | "You referenced required evidence but did not run the required tool(s): {missing_tools}. Run them and regenerate the thesis using those observations." |
| Source-code path read trace for `source_code_verification`. | process | attempt-trace extension | `process_source_code_not_read`, `process_source_code_path_not_read` | add | Extract the repo-relative path from `source_code_verification`. Require at least one source-reading tool call in the attempt trace. Require the extracted path to appear in normalized read paths. Do not treat a valid-looking citation string as proof the file was read. | `process_source_code_not_read`: "You cited source code but did not read any source file in this attempt." `process_source_code_path_not_read`: "You cited {path}, but that path was not read in this attempt. Read the cited file or cite the file you actually inspected." |
...
| Strategy family missing `key_concepts`. | compiler contract | `validate_stage_2` | `hypothesis_config_alignment_unconfigured` | keep | If a family should enforce Stage 2 alignment but has no `key_concepts`, emit an explicit configuration failure rather than silent success. Temporary fail-open must be limited to declared scaffold/demo families. | "Stage 2 alignment cannot run because this strategy family has no key_concepts map. Add family key concepts or mark the family as intentionally unenforced." |
...
| Gate | Scope | Owner | Rejection code(s) | Status | Logic | Feedback to research conductor |
|---|---|---|---|---|---|---|
| Expected effects pass/fail against backtest metrics. | evaluator | evaluator / `BacktestVerdict` | `passed_effects`, `failed_effects` verdict fields | keep outside Stage 1 | After a run, compare actual metrics against each `expected_effects` direction and magnitude range where provided. Report pass/fail per effect; do not Stage 1 reject before runtime. | "Post-run result: expected effect {metric} {direction} {range} was {passed_or_failed} with actual value {actual_value}." |
| Disqualifiers triggered by run results. | evaluator | evaluator / `BacktestVerdict` | `triggered_disqualifiers`, `unparsed_disqualifiers` verdict fields | keep outside Stage 1 | Parse each disqualifier condition that can be evaluated against run metrics/diagnostics. Mark triggered disqualifiers; separately report unparsed ones for prompt/spec improvement. | "Post-run disqualifier {name} triggered: {condition} matched actual diagnostics. Treat the thesis as killed unless a human overrides." |
...
| Gate | Scope | Owner | Failure mode | Status | Logic | Feedback |
|---|---|---|---|---|---|---|
| `ResearchThesis.model_fields` match rendered OUTPUT fields except `_PROMPT_OMITTED_FIELDS`. | drift | `scripts/check_prompt_drift.py` | CI failure | keep | Introspect schema fields and rendered OUTPUT headings. Fail if any schema field is missing from rendered OUTPUT or any rendered field is absent from schema, except explicitly omitted system/compiler fields. | "Drift check failed: schema field/rendered OUTPUT mismatch: {field}." |
| Validator rejection codes match `prompts/conductor_output_rules.json` except `_PROMPT_OMITTED_RULES`. | drift | `scripts/check_prompt_drift.py` | CI failure | add | Extract rejection codes emitted by validators and compare with generated sidecar. Fail if a validator emits a code with no sidecar rule or a sidecar rule has no validator implementation, unless omitted. | "Drift check failed: rejection code {code} is not synchronized between validator and rules sidecar." |
