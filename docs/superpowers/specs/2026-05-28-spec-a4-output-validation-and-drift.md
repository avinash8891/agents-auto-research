...
| `thesis_role` required literal. | `(O)` | Pydantic plus generated predicate | `structural_thesis_role_required` | add | `always_required` | Require one of A4a's three literals: `orthogonal_discovery`, `implementation_unlock`, `cleanup_validation_follow_up`. | "Set `thesis_role` to one of the allowed role labels." |
