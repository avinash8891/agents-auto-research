...
3. **DOCTRINE ↔ OUTPUT duplication** — soft principles imply concrete typed-field contracts, creating a second prompt surface that can drift from OUTPUT.
...
The one permitted bridge is OUTPUT-side annotation generated by the §7
...
5. **`research_prompts.py`** (or `autoresearch_research.py`):
   ...
   - DOCTRINE stripped of schema-field names and typed contracts; old OUTPUT prose deleted.
6. **DOCTRINE backref mapping** (per §6):
   ...
   - CI gate: every key in the backref mapping must be a real field; DOCTRINE must contain no `ResearchThesis.model_fields` names and no `→ see <field>` references.
...
14. **`tests/test_conductor_prompt_v3.py`**:
    ...
    - DOCTRINE backref mapping covers every OUTPUT field whose Producer guidance has a DOCTRINE annotation.
    - DOCTRINE prose contains no `ResearchThesis.model_fields` names and no `→ see <field>` references.
...
- DOCTRINE contains no schema-field names, typed shapes, enum values, counts, requiredness, or validator behavior.
- Every OUTPUT entry's `Producer guidance` has a generated `(DOCTRINE: <principle>)` annotation where applicable.
...
- DOCTRINE rewrites beyond removing schema-field contracts and keeping principles field-agnostic.
