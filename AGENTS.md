...
6. **Tests run and pass.** Paste actual test-runner output into the completion message.
   - Default verification for the full suite is GitHub Actions, not the local machine.
   - For normal code changes: commit, push the current branch, watch the `CI` workflow with `gh run watch --exit-status`, and report the run URL plus the final test summary.
   - Local test runs are allowed for cheap targeted checks, formatting, or debugging a failing CI run. Do not run the full suite locally unless the user explicitly asks.
...
- **B. One home per concept.** Each external service client, each shared utility, each data-model file — one location, declared up front. Nobody re-instantiates locally. Data models live in one file; adding or changing a field is announced, not silent. Payload normalizers (e.g. `normalize_thesis_payload`) must be public and invoked at *every* `model_validate` call site — if a second caller appears, promote the helper out of `_private` rather than duplicating the normalization logic. Skipping normalization at one site means valid payloads will raise `ValidationError` and abort whatever flow that site drives.
...
- **Terminal-state bookkeeping precedes deterministic-error validation.** If a handler decides the run reached a terminal state (`halted`, `manual_review`, `finished`), the state mutation + operator notification (`_close_run`, Discord, etc.) must run *before* any code that can raise. A `ValidationError` propagating out of a halt handler before the halt fields are written leaves the controller in an inconsistent partial state, bypasses the operator notification, and is indistinguishable from a crash. Pattern: write halt fields first, then validate/compile as best-effort enrichment, with their own `try/except` that logs and continues.
...
```bash
# Tests
pytest                                        # targeted/local debugging only; full suite runs in GitHub Actions
pytest --cov --cov-report=term-missing        # targeted/local debugging only; CI enforces the coverage gate
