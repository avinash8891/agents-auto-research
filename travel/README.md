...
4. Per theme: saturate operators across every baseline axis (`axes-registry.md`) + verify finalists → rank Top `RANK_DEPTH` (`ranking`). Operator-converged = `OPERATOR_CONVERGED` (`travel-config.md`): every baseline axis returns dry AND the theme carries no `dirty` flag.
