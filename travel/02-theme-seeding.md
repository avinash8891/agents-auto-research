...
3. For each region, draft candidate subjects from live-known country facts and one or more **lenses** taken from `lens-registry.md` (baseline + candidates). Do not use an inline/remembered lens list — the registry is the source of truth. This step proposes search targets only; `admission-bar` decides representative/admitted status from live evidence.
4. **Archetype walk**: go through `theme-archetypes.md` and ask, for this country, "does this archetype have a candidate match?" Instantiate matches as candidate themes; note misses. (Catches patterns free-recall skips — e.g. "wine region", "wildlife circuit", "pilgrimage circuit".) The admission rubric later labels strong/watch/fail.
...
## DECISION RULES
- SEED DOES NOT ADMIT: seed rows are search candidates only. Admission, representative fit, and thin/failed status are assigned by `admission-bar` using live evidence.
