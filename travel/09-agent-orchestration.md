...
5. Compute the fleet size from scope, not taste: `fleet = min(MAX_AGENTS_PER_COUNTRY, max(1, number of macro-region clusters or independent theme groups in this batch))`. If the task is a convergence/axis-proof run, use one agent per required axis from the `axes-registry.md` tag filter, still capped by `MAX_AGENTS_PER_COUNTRY`; batches beyond the cap queue to the next round.
...
9. Treat every returned finding as a **hypothesis** (lesson `L7`, `lessons`): spot-check named guides / `CURRENT_SEASON`-dated departures against a live URL before any operator enters a ranking. This orchestrator spot-check is **SAMPLING for the broad corpus only** — it does NOT suffice before ranking. The authoritative **100%-finalist re-verify gate** (every displayed finalist re-fetched against a live URL, no exceptions) is owned by `verification` step 4 (`07-verification-and-ranking.md`); 09 DEFERS to it and never substitutes its sampling spot-check for that full gate (`C15`).
...
- A finding enters a ranking IFF its named guide/date was spot-checked against a live URL (verify, don't trust). This spot-check is SAMPLING for the broad corpus only — it is NOT the finalist gate.
- A DISPLAYED finalist ships IFF it passed the 100%-finalist re-verify owned by `verification` step 4 (every finalist re-fetched live, no exceptions). 09's sampling spot-check NEVER substitutes for that gate (`C15`).
- One file per agent IFF parallel — shared file → write collisions.
- Fleet size is valid IFF it follows PROCEDURE B.5 and the budget dials in `travel-config.md`; otherwise resize or queue the excess batch.
...
BUDGET DIALS (`C11`; DEFINED in `travel-config.md` COST/BUDGET, REFERENCED here): fan-out and loop-until-dry are bounded by the COST/BUDGET dials — `MAX_AGENTS_PER_COUNTRY` (cap on fleet size in `B`/DISPATCH), `MAX_DISCOVERY_ROUNDS` (cap on round count), `MAX_SEARCHES_PER_AXIS` (the operational replacement for the open-ended "do NOT stop at a handful" in the discovery template), and `PER_COUNTRY_SEARCH_BUDGET` (aggregate search ceiling per country). loop-until-dry terminates on EITHER `OPERATOR_CONVERGED` (dry) OR budget-exhausted-with-logged-residual — the explicit **NON-CONVERGENCE STOP**. Budget-exhausted-before-convergence ⇒ the artifact is stamped **INCOMPLETE** (which axes/regions/themes were left unfinished). The fixed-point termination proof (`registry-protocol` INVALIDATION; `config` DONE) is NOT a substitute for this operational budget — it proves convergence terminates, the budget bounds cost.

SCALE NOTE (rank-aware sweeps + global operator/authority cache — named upgrade path, NOT built at pilot scale; review-findings C8): at `CURRENT_SCOPE_N` (10 countries) the per-theme saturate-then-rank and per-country directory mining above are affordable, so they stay as written and the BUDGET DIALS bound them. At `GROWTH_LADDER` 50–150 the same pattern multiplies: each theme re-mines the same authority directories (O(themes)), and the global authority/membership directories (Virtuoso/AITO/ETOA/museum-partner + the destination-side registers in `sources-registry.md`) are re-mined per country (O(countries)). The deferred restructuring is: (a) saturate operators ONCE at country level and have ranking READ the converged corpus rows per theme (live web work reserved for verifying finalists' specifics), batching ranking by CLUSTER not per theme; (b) a first-class GLOBAL operator + authority-directory cache — mine each directory once, tag operators with countries/themes served, filter to a new country before any fresh mining; (c) gate cross-country dirtying so a promotion dirties priors ONLY with cross-country evidence it would change a ranked Top-`RANK_DEPTH`. This is a SCALE optimization (like the embeddings dedup index above), not a pilot fix — do NOT build it at `CURRENT_SCOPE_N` (YAGNI); revisit when coverage outgrows the budget.
...
- Fleet size not computed from PROCEDURE B.5, or exceeding `MAX_AGENTS_PER_COUNTRY` without queueing the excess batch.
...
- Treating the orchestrator's sampling spot-check (PROCEDURE D.9) as the finalist gate, or shipping displayed finalists without the 100%-finalist re-verify owned by `verification` step 4 → unverified finalists reach the published list (`C15`; `07-verification-and-ranking.md` step 4 is authoritative, 09 defers).
- Writing the Italy legacy path `italy/theme_map_v0.md` / `italy/theme_map_FINAL.md` (no `italy_` prefix) → that file does not exist on disk; Italy's pre-scheme files keep the redundant `italy_` prefix (`italy/italy_theme_map_v0.md` … `italy/italy_theme_map_FINAL.md`, legacy corpus `italy/italy_discovery_corpus.md`) (`C16`).
- Fanning out or looping-until-dry with no budget bound — exceeding `MAX_AGENTS_PER_COUNTRY` / `MAX_DISCOVERY_ROUNDS` / `MAX_SEARCHES_PER_AXIS` / `PER_COUNTRY_SEARCH_BUDGET`, or treating the fixed-point proof as if it bounded cost → runaway discovery spend (`C11`; `travel-config.md` COST/BUDGET).
- Budget-exhausting before convergence and NOT stamping the artifact INCOMPLETE (with the unfinished axes/regions/themes) or NOT logging the residual at the NON-CONVERGENCE STOP → a thin run masquerades as converged (`C11`).
