...
| credential fit | named expert's credential independently corroborated and resolves to the theme lens in `lens-registry.md` | seller page claims a plausible credential only | credential exists but lens fit is adjacent/weak | no named expert, generic guide only, or `credential-mismatch` |
| depth | itinerary shows multiple theme-specific sites/access/interpretation points | seller-only depth claim with concrete itinerary evidence | theme appears as one segment in a broader tour | generic sightseeing/adjectives only |
| reputation | independent institutional bio, publication/register, serious press, award, or review corpus supports operator/expert seriousness | third-party directory/review only, or seller-only reputation | weak/stale/unclear independent mention | no independent signal |
| format | matches the ranking lane exactly | minor mismatch disclosed in FLAGS | major mismatch; closest-fit only | wrong product type for this ranking |
| value | price justified by verified expertise, access, small group, duration, or rare logistics | plausible but not independently supported | premium with weak support | premium-for-thin-substance |
