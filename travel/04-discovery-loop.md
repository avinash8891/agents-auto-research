...
- **Completeness-critic rounds**: adversarial agents whose only job is to find what's missing — one for LENSES, one for CHANNELS/operators, one for REGIONS, one for borderline validation. These catch the long tail.
  ...
  - **Axis-completeness critic**: one critic asks not "what operator/theme did we miss?" but **"what DIMENSION are we blind to?"** — test the candidate axes from `axes-registry.md` (format/vehicle, affinity, media/creator, season, price-tier) for this country. If a candidate provably surfaces tours no existing axis finds, PROMOTE it (record in `<country>/axes.md`, escalate to the global registry). The axis set is itself a convergence target (`03`).
