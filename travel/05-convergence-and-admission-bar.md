...
- COUNTRY IS DONE IFF BOTH: (a) `THEME_CONVERGED` — a fresh adversarial completeness-critic admits 0 themes clearing `ADMISSION_BAR`; AND (b) `OPERATOR_CONVERGED` — every BASELINE axis (`axes-registry.md`, count derived) returns dry for that theme (no new credentialed operator) AND the theme carries no `dirty` flag.
