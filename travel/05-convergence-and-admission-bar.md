...
- [C9] AVAILABILITY-BIAS GATE (coverage ≠ best): "dry" = search EXHAUSTION, NOT field completeness. Before declaring a theme converged, require all three checks: (1) at least one destination-language sweep; (2) at least one destination-platform or destination-authority sweep; (3) every empty convergence-gate axis has a written reason: `market-empty`, `source-base-limited`, or `budget-exhausted`. `source-base-limited` or `budget-exhausted` raises a `coverage-limitation` FLAG and makes convergence PROVISIONAL. State the basis: "dry = search exhaustion, not field completeness."
...
- Declaring a theme converged when a convergence-gate axis is empty due to SOURCE coverage (not market) — that is search exhaustion mistaken for field completeness; missing any C9 availability-bias gate check. [C9]
