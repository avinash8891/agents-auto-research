...
## L28 — Ran the audit; a finder-only sweep is non-exhaustive (grep backstop required)
**Did:** ran `AUDIT-CHECKLIST.md` (the 17 classes, L27) as a multi-agent workflow — one finder per class across all 24 docs, each finding adversarially re-verified. Surfaced 37 real residual instances (after killing 12 false-positives); fixed all, then independently re-verified 45/45 resolved with zero regressions.
**Why the instances existed:** the method's own conventions outran their back-propagation — each landed on the doc *in focus* and the **reference docs (the 7 registries + `travel-config`) lagged**. Slugs (L16/L17) post-dated the registries → they still cited step docs by bare number; dirty-propagation (L19) was written axis-only → `channel` was named in the INVALIDATION header but had no re-sweep case; versioned-schema (L21) covered only the corpus row → `country_ranking`/`rankings` schemas never got it; ledger consolidation (L20) left a stale `_changelog` path + a wrong `corpus_FINAL` producer attribution in `admission-bar`.
**The real lesson — finders miss; greps don't:** the class-10 finder found 9 bare-number refs but **missed ~6** (`travel-config` 01/11/08/06, `README`, `REGISTRY-PROTOCOL`, `axes-registry` UPDATE). A deterministic `grep` over the whole corpus caught every remaining one. Fixing also surfaced two self-introduced gaps (an orphan `` `DONE` `` ref with no dial; a region read-before-write ordering) — caught by the same grep/review backstop.
**Fix:** 37 audited + follow-ups across 22 files. Notable: `DONE` promoted to a named `travel-config.md` dial; `channel`-sub-type INVALIDATION case added (REGISTRY-PROTOCOL + `discovery-loop` + `orchestration`); `withdrawn` added to `tags-registry` row.status; the `region` axis given a committed home (`<country>/axes.md`, first-populated by `theme-seeding`); `admission-bar` now reads per-round corpus files (not the freshness-produced `corpus_FINAL`). `10-lessons-log` itself left untouched (historical record).
**Rules going forward:** (1) pair every audit finder with a mechanical grep backstop — a finder alone is itself issue-class #2 (non-exhaustive). (2) The doc-currency guard (L25) + static-census must include the **registries + config**, not just step docs — that is where convention-lag accumulates.
