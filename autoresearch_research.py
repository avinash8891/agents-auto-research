...
def _try_one_validation_attempt(
    ...
    try:
        ...
        _log_validation_rejection(
            controller, research_round, attempt, raw_thesis, thesis_id,
            str(exc), exc=exc, stage="stage_1",
        )
...
def _dispatch_compiled_contract(
    ...
    setattr(
        compile_exc,
        "evidence",
        {"contract_status": contract.status, "missing_primitives": list(contract.missing_primitives)},
    )
    _log_validation_rejection(
        controller,
        research_round,
