...
def normalize_contract(contract: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for primitive in contract:
        canonical_type = CANONICAL_PRIMITIVE_ALIASES.get(primitive.get("type"), primitive.get("type"))
...
def render_contract_to_runtime_config(contract: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for primitive in contract:
        ...
        if primitive_type == "universe_stocks_in_play":
            ...
            rendered["time_stop_minute"] = primitive.get("minute", primitive.get("time_stop_minute", 0))
            ...
            rendered["rvol_lookback_days"] = primitive.get("lookback_days", primitive.get("rvol_lookback_days"))
            baseline_aliases = {"sma": "mean", "ema": "median"}
            raw_baseline = primitive.get("baseline_stat", primitive.get("baseline", primitive.get("rvol_baseline_stat")))
            ...
            if "min_history_days" in primitive or "rvol_min_history_days" in primitive:
                rendered["rvol_min_history_days"] = primitive.get("min_history_days", primitive.get("rvol_min_history_days"))
