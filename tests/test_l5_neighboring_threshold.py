...
def _base_thesis(thesis_id: str, config_changes: dict) -> dict:
    return {
        ...
        "expected_effects": [
            {"metric": "profit_factor", "direction": "increase", "rationale": "fewer regime-mismatched trades"},
            {"metric": "trade_count", "direction": "decrease_or_same", "rationale": "selective filter"},
        ],
        "disqualifiers": [
            {"name": "trade_count_collapse", "condition": "trade_count drops more than 50%", "kind": "metric_threshold"},
            {
...
def test_validator_rejects_neighboring_threshold_same_key_small_delta() -> None:
    ...
    prior = [
        _prior(
            "prior_gap_filter",
            {"gap_exclude_pct": 0.01, "prior_only_a": True, "prior_only_b": False, "prior_only_c": "x"},
        )
    ]
