...
def test_process_gate_warns_when_web_search_not_called(caplog: pytest.LogCaptureFixture) -> None:
    thesis = validate_thesis_dict(_valid_thesis(), tools_called={"list_round_results"})

    assert thesis.thesis_id == "job-test-round-1-attempt-1"
    assert "required tools not called: ['web_search']" in caplog.text


def test_process_gate_warns_when_experiment_results_not_called(
    caplog: pytest.LogCaptureFixture,
) -> None:
    thesis = validate_thesis_dict(_valid_thesis(), tools_called={"web_search"})

    assert thesis.thesis_id == "job-test-round-1-attempt-1"
    assert "required tools not called: ['list_round_results']" in caplog.text


def test_process_gate_warns_with_multiple_missing_tools(
    caplog: pytest.LogCaptureFixture,
) -> None:
    thesis = validate_thesis_dict(_valid_thesis(), tools_called=set())

    assert thesis.thesis_id == "job-test-round-1-attempt-1"
    assert "required tools not called: ['list_round_results', 'web_search']" in caplog.text
...
def test_process_gate_warns_when_analyst_tool_required(
    caplog: pytest.LogCaptureFixture,
) -> None:
    thesis = validate_thesis_dict(
        _valid_thesis(),
    ...
    assert thesis.thesis_id == "job-test-round-1-attempt-1"
    assert "required tools not called: ['analyze_trades']" in caplog.text
