...
# DEAD CODE — `subprocess` and `trace_agent_tool_call` were only used by the
# commented-out `_run_diagnostic_analyst_openai` below (superseded by
# research_subagents._call_analyst). Re-add to the live imports above when /
# if that function is brought back.
# import subprocess
# from trace_sdk import trace_agent_tool_call
...
# DEAD CODE — diagnostic analyst entry point. Superseded by
# research_subagents._call_analyst, which is the live agent the conductor
# dispatches via its analyze_trades tool. Kept commented out temporarily;
# delete after the live analyst absorbs the missing behaviors (mempalace
# auto-persist, prior-diagnostics recall, optional broad-sweep mode).
