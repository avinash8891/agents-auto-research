...
def classify_regimes(
    ...
    for name, df in [
        ("trend", trend_mask),
        ("chop", chop_mask),
        ("high-vol", high_vol_mask),
        ("low-vol", low_vol_mask),
        ("wide-OR", wide_or_mask),
        ("narrow-OR", narrow_or_mask),
    ]:
...
def apply_regime_gate(
    ...
    entry_dates = (
        ...
        if hasattr(trades_df["entry_date"].iloc[0], "date")
        else trades_df["entry_date"]
    )
